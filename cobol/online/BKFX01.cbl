      *****************************************************************
      * PROGRAM-ID : BKFX01
      * TITLE      : Foreign check deposit — holds 5 days, channel TELLER, CCY not USD
      * Synthetic lab system. Not a real bank. Educational / portfolio only.
      * Places hold; credits BAL immediately only if CCY=USD else hold only.
      *****************************************************************
       ...
       2000-EDIT.
           IF ACC-CCY NOT = 'USD'
              MOVE 'FX-HOLD' TO HLD-REASON
           END-IF
           EXIT.
       3000-WORK.
           MOVE JRN-AMT TO HLD-AMT
           SET HLD-ACTIVE TO TRUE
           EXEC SQL INSERT INTO BANK_HOLD
             (HOLD_ID, ACCOUNT_NO, AMOUNT, REASON, EXP_DTE, STATUS)
             VALUES (:HLD-ID, :ACC-NO, :HLD-AMT, 'FX-HOLD',
                     CURRENT DATE + 5 DAYS, 'A')
           END-EXEC
           EXEC SQL UPDATE BANK_CUSTOMER
             SET HOLD_AMT = HOLD_AMT + :HLD-AMT,
                 BALANCE = BALANCE + :JRN-AMT
