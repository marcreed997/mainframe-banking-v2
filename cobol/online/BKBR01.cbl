      *****************************************************************
      * PROGRAM-ID : BKBR01
      * TITLE      : Branch totals inquiry — vault + teller drawers vs in-transit
      ...
      *****************************************************************
       ...
       0000-MAIN.
           ...
           .
       1000-RECV.
           EXEC CICS RECEIVE MAP('BKBRMP') MAPSET('BKBRMS')
       ...
       2000-EDIT.
           EXIT.
       3000-WORK.
           EXEC SQL SELECT VAULT_CASH, TELLER_TOT, IN_TRANSIT
             INTO :BRN-VAULT, :BRN-TELLER-TOT, :BRN-IN-TRANSIT
             FROM BANK_BRANCH_CASH WHERE BRANCH_NO = :BRN-NO
           END-EXEC
           PERFORM 8000-MAP-SQL
           EXIT.
       4000-SEND.
           EXEC CICS SEND MAP('BKBRMP') MAPSET('BKBRMS') ERASE
                RESP(WS-RESP)
           END-EXEC
           PERFORM 8100-CICS-RESP
           EXIT.
